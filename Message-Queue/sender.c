...
int main(int argc, char **argv){
    ...
    queue = mq_open(argv[1], O_WRONLY | O_CREAT, 0660, NULL); // Open messga queue with default attributes
    if(queue == (mqd_t)-1){
        perror("mq_open");
        return 1;
    }
    ...
    if (mq_send(queue, argv[2], strlen(argv[2]), 0) == -1){  // 0 is the priority that can be set based on message priority is least priority
        perror("mq_send");
        mq_close(queue);
        return 1;
